<?php
	...
	try {
		...
		if($vendorFound == 1){
			//echo "true";
			$query2 = $db->prepare("select * from chats where chat_id like '%$vendorId%' and seen=0 order by date_time desc");
			$query2->execute();
			$result2 = $query2->fetchAll();
			$chatIds = array();
			$dateTimes = array();
			...
			foreach($uniqueChatIds as $val){
				$query3 = $db->prepare("select date_time from chats where chat_id='$val' and seen_vendor=0 order by date_time desc");
				$query3->execute();
