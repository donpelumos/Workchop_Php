<?php
	...
	try {
		...
		$query2 = $db->prepare("insert into issues_vendor (unique_id, vendor_id, subject, issue, date_time) values(:unique, :vendor_id, :subject, :issue, :date_time)");
		
		$query2->bindParam(':vendor_id',$vendorId);
		$query2->bindParam(':subject',$subject);
		$query2->bindParam(':issue',$issue);
		...
		$issue = strip_tags(trim($_GET['issue']));		
